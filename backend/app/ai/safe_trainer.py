...
def _split_frame(
    ...
    """Build a symbol dataset and split it strictly by row order."""
...
def train_from_dataframe_safe(
    ...
    # Each symbol is split independently; no symbol's future is allowed into
    # its own training set. This also prevents concatenation order from
    # becoming a hidden validation leak.
    dense_parts = []
    for df in dfs:
        xtr, ytr, xva, yva = _split_frame(
            df, horizon, atr_mult, val_fraction, purge=horizon
        )
    ...
    if model_type in {"lstm", "ensemble"}:
        ...
        for df in dfs:
            ...
            # Windows are generated after the split, so no validation timestep
            # can appear inside a training sequence.
            tr_x, tr_y = raw_x[split.train], raw_y[split.train]
            va_x, va_y = raw_x[split.validation], raw_y[split.validation]
            if len(tr_x) <= lstm_seq_len or len(va_x) <= 1:
                continue
        ...
        if seq_train:
            ...
            Xs_tr_s = seq_scaler.fit_transform(Xs_tr.reshape(-1, f)).reshape(n, t, f).astype(np.float32)
            nv = Xs_val.shape[0]
            Xs_val_s = seq_scaler.transform(Xs_val.reshape(-1, f)).reshape(nv, t, f).astype(np.float32)
            # Dense and LSTM scalers are persisted separately; Predictor's
            # existing contract uses scaler.joblib, so dense remains canonical.
            ...
            joblib.dump(seq_scaler, out_dir / "lstm_scaler.joblib")

    meta = {
        "features": legacy.FEATURE_NAMES,
        "n_classes": legacy._N_CLASSES,
        "horizon": horizon,
        "atr_mult": atr_mult,
