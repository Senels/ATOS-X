"""Detailed analysis of top configs"""
...
def analyze(adx_th, dsp, tpbm, tpm, mdca, atr_l, name):
    ...
    for i in range(warmup,n):
        ...
        if TPR and AD!=0:
            ...
            EQ+=dpnl; TRADES.append({'pnl':dpnl/EQ_prev if 'EQ_prev' in dir() else 0,'win':True,'dca':DL,'reason':'tp','dir':AD})
            AD=0; TPR=False; DL=1
        ...
        if rev and AD!=0:
            ...
            EQ+=dpnl; TRADES.append({'pnl':dpnl/EQ_prev if 'EQ_prev' in dir() else 0,'win':won,'dca':DL,'reason':'rev','dir':AD})
            AD=0; TPR=False; DL=1
    ...
    cret=(EQ/10000-1)*100
    gp=sum(abs(t['pnl']) for t in TRADES if t['pnl']>0); gl=sum(abs(t['pnl']) for t in TRADES if t['pnl']<0)
    pf=gp/gl if gl>0 else 999
    dca_dist={}; rev_by_dca={}
    for t in TRADES:
        lvl=t['dca']; dca_dist[lvl]=dca_dist.get(lvl,0)+1
        if t['reason']=='rev': rev_by_dca[lvl]=rev_by_dca.get(lvl,0)+1
    losses_by_dca={}
    for t in TRADES:
        if not t['win']: losses_by_dca[t['dca']]=losses_by_dca.get(t['dca'],0)+1
    print(f"\n=== {name} ===")
    print(f"Trades: {len(TRADES)}  Wins: {wins} ({wins/len(TRADES)*100:.1f}%)")
    print(f"Compound Return: {cret:+.2f}%  PF: {pf:.2f}  MaxDD: {MAX_DD:.1f}%")
    print(f"DCA: {dict(sorted(dca_dist.items()))}")
    print(f"Rev by DCA: {dict(sorted(rev_by_dca.items()))}")
    print(f"Losses by DCA: {dict(sorted(losses_by_dca.items()))}")
    # worst trades
    worst=sorted(TRADES, key=lambda x: x['pnl'])[:3]
    for w in worst:
        print(f"  Worst: {'L' if w['dir']==1 else 'S'} pnl={w['pnl']*100:.2f}% DCA={w['dca']} {w['reason']}")
    best=sorted(TRADES, key=lambda x: x['pnl'], reverse=True)[:3]
    for b in best:
        print(f"  Best: {'L' if b['dir']==1 else 'S'} pnl={b['pnl']*100:.2f}% DCA={b['dca']} {b['reason']}")

analyze(35, 3.0, 1.5, 1.5, 3, 10, "DCA3 ADX35 dsp3.0 TP1.5x1.5 ATR10")
analyze(35, 1.5, 1.5, 1.5, 3, 10, "DCA3 ADX35 dsp1.5 TP1.5x1.5 ATR10")
analyze(35, 3.0, 1.5, 1.5, 3, 14, "DCA3 ADX35 dsp3.0 TP1.5x1.5 ATR14")
analyze(35, 2.0, 1.5, 1.5, 3, 10, "DCA3 ADX35 dsp2.0 TP1.5x1.5 ATR10")
