"""
Parameter sweep - fixed compound return calculation
"""
...
def run(adx_thresh, dca_spacing, tp_base_mult, tp_mult, max_dca_level, atr_len):
    ...
    for i in range(warmup,n):
        ...
        if TPR and AD!=0:
            ...
            dpnl=(ATP-avg)*tq if AD==1 else (avg-ATP)*tq; EQ+=dpnl; TRADES.append({"pnl":dpnl,"win":True})
            AD=0; TPR=False; DL=1
        ...
        if rev and AD!=0:
            ...
            TRADES.append({"pnl":dpnl,"win":won})
            AD=0; TPR=False; DL=1
    ...
    compound_return = (EQ / 10000.0 - 1) * 100
    return len(TRADES), wins/len(TRADES)*100, compound_return, pf, gp, gl, MAX_DD, wins

print(f"{'Params':<60} {'Trades':>6} {'WR%':>5} {'Ret%':>8} {'PF':>6} {'MaxDD':>7}")
print("-"*100)
results = []
for max_dca in [3, 4, 5]:
    for adx in [25, 30, 35]:
        for dsp in [0.5, 1.0, 1.5, 2.0, 3.0]:
            for tpbm in [1.5, 2.0]:
                for tpm in [1.0, 1.5, 2.0]:
                    for atr in [10, 14]:
                        t, wr, cret, pf, gp, gl, mdd, w = run(adx, dsp, tpbm, tpm, max_dca, atr)
                        if t == 0: continue
                        results.append((max_dca, adx, dsp, tpbm, tpm, atr, t, wr, cret, pf, mdd))

results.sort(key=lambda r: r[9], reverse=True)
for r in results[:30]:
    md, adx, dsp, tpbm, tpm, atr, t, wr, cret, pf, mdd = r
    label = f"DCA{md} ADX>{adx} dsp={dsp} TP={tpbm}x{tpm} ATR{atr}"
    print(f"{label:<60} {t:6d} {wr:5.1f} {cret:+8.2f} {pf:6.2f} {mdd:7.1f}")
