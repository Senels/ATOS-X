"""
Parameter sweep for Legend BUY SELL DCA
"""
...
def run(adx_thresh, dca_spacing, tp_base_mult, tp_mult, risk_pct, max_dca_level, atr_len):
    atr_vals=rma(tr,atr_len)
    bullish=(adx_val>adx_thresh)&macd_cross_up
    bearish=(adx_val>adx_thresh)&macd_cross_down
    AD=0; AE=0.0; ATP=0.0; DL=1; DBE=0.0; EBQ=0.0; EB=-999; TPR=False; EQ=10000.0; TRADES=[]
    warmup=max(atr_len,14,13)*3
    ...
    for i in range(warmup,n):
        ...
        if rev and AD!=0:
            ...
            TRADES.append({"pnl":ppnl,"win":won,"dca":DL,"reason":"rev"})
            AD=0; TPR=False; DL=1
            if bearish[i]: AD=-1; AE=c[i]; ATP=c[i]-tpt; DBE=c[i]; DL=1; EBQ=eqty(EQ,c[i],risk_pct,tpd); EB=i
            elif bullish[i]: AD=1; AE=c[i]; ATP=c[i]+tpt; DBE=c[i]; DL=1; EBQ=eqty(EQ,c[i],risk_pct,tpd); EB=i
        if AD==0:
            if bullish[i]: AD=1; AE=c[i]; ATP=c[i]+tpt; DBE=c[i]; DL=1; EBQ=eqty(EQ,c[i],risk_pct,tpd); EB=i
            elif bearish[i]: AD=-1; AE=c[i]; ATP=c[i]-tpt; DBE=c[i]; DL=1; EBQ=eqty(EQ,c[i],risk_pct,tpd); EB=i
    if not TRADES: return 0,0,0,0,0,0,0
    wins=sum(1 for t in TRADES if t["win"]); tot=sum(t["pnl"] for t in TRADES)
    gp=sum(t["pnl"] for t in TRADES if t["pnl"]>0); gl=abs(sum(t["pnl"] for t in TRADES if t["pnl"]<0))
    pf=gp/gl if gl>0 else 999
    return len(TRADES), wins/len(TRADES)*100, tot, pf

print(f"{'Params':<55} {'Trades':>6} {'WR%':>5} {'NetP%':>8} {'PF':>6}")
print("-"*85)
best_pf = 0
best_params = None
results = []
for adx in [25, 30, 35]:
    for dsp in [0.5, 1.0, 1.5, 2.0]:
        for tpbm in [1.5, 2.0, 2.5]:
            for tpm in [1.0, 1.5, 2.0]:
                for atr in [10, 14, 20]:
                    t, wr, npnl, pf = run(adx, dsp, tpbm, tpm, 1.0, 5, atr)
                    results.append((adx, dsp, tpbm, tpm, atr, t, wr, npnl, pf))

# Sort by PF desc
results.sort(key=lambda r: r[8], reverse=True)
for r in results[:20]:
    adx, dsp, tpbm, tpm, atr, t, wr, npnl, pf = r
    label = f"ADX>{adx} DCA={dsp} TP={tpbm}x{tpm} ATR{atr}"
    print(f"{label:<55} {t:6d} {wr:5.1f} {npnl:+8.2f} {pf:6.2f}")
